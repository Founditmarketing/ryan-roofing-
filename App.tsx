...
function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-black font-sans text-neutral-200">
      {/* Header */}
      <header className={`fixed top-0 w-full z-40 text-white transition-all duration-300 ${scrolled ? 'bg-black/95 shadow-xl border-b border-white/10 backdrop-blur-md' : 'bg-transparent border-transparent'}`}>
        <div className={`w-full px-6 md:px-12 flex items-center justify-between transition-all duration-300 ${scrolled ? 'h-20 md:h-24' : 'h-28 md:h-40'}`}>
          <div
            className="flex items-center gap-3 cursor-pointer group"
            onClick={() => setCurrentView(PageView.HOME)}
          >
            <div className={`flex items-center justify-center group-hover:scale-105 transition-all duration-300 ${scrolled ? 'w-16 h-16 md:w-20 md:h-20' : 'w-32 h-32 md:w-48 md:h-48'}`}>
              <img src="/ryanconstructionlogo.png" alt="Ryan Construction Logo" className="w-full h-full object-contain" />
            </div>
          </div>
